...
// monitor the stack size in the code to check to make sure I'm not too close
// to overflowing the stack This monitoring takes place if INPSECT_STACK is
// defined (search this file for INSPECT_STACK to see the code for this)
...
//   I do not want to access the message buffer data structures outside of these
// routines
...
// If LCD_EXAMPLE_OP=0, then accept messages that may be timer or print requests
...
static unsigned short hsl2rgb(float H, float S, float L)
{
  ...
  G1 += m;
  B1 += m;
  unsigned short red = R1 * 32;
  if (red > 31) red = 31;
  unsigned short green = G1 * 64;
  if (green > 63) green = 63;
  unsigned short blue = B1 * 32;
  if (blue > 31) blue = 31;
