...
typedef struct __vtTempMsg
{
  ...
  uint8_t buf[vtTempMaxLen + 1];  // On the way in, message to be sent, on the
                                  // way out, message received (if any)
} vtTempMsg;

// I have set this to a large stack size because of (a) using printf() and (b)
// the depth of function calls
//   for some of the i2c operations	-- almost certainly too large, see
// LCDTask.c for details on how to check the size
#define baseStack 3
#if PRINTF_VERSION == 1
...
/*-----------------------------------------------------------*/
// Public API
void vStarti2cTempTask(vtTempStruct *params,
                       ...
                       vtLCDStruct *lcd)
{
  ...
  params->dev = i2c;
  params->lcdData = lcd;
  if ((retval = xTaskCreate(vi2cTempUpdateTask,
                            (signed char *)"i2cTemp",
                            i2cSTACK_SIZE,
                            (void *)params,
                            uxPriority,
...
static portTASK_FUNCTION(vi2cTempUpdateTask, pvParameters)
{
  ...
  // This task is implemented as a Finite State Machine.  The incoming messages
  // are examined to see
  ...
  for (;;) {
    ...
    if (xQueueReceive(param->inQ, (void *)&msgBuffer, portMAX_DELAY) !=
        pdTRUE) {
      VT_HANDLE_FATAL_ERROR(0);
    }

    // Now, based on the type of the message and the state, we decide on the new
    // state and action to take
    switch (getMsgType(&msgBuffer)) {
    case vtI2CMsgTypeTempInit: {
      if (currentState == fsmStateInit1Sent) {
        currentState = fsmStateInit2Sent;
        // Must wait 10ms after writing to the temperature sensor's
        // configuration registers(per sensor data sheet)
        vTaskDelay(10 / portTICK_RATE_MS);
        // Tell it to start converting
        if (vtI2CEnQ(devPtr,
                     vtI2CMsgTypeTempInit,
                     0x4F,
                     sizeof(i2cCmdStartConvert),
                     i2cCmdStartConvert,
                     0) != pdTRUE) {
          VT_HANDLE_FATAL_ERROR(0);
        }
    ...
    case TempMsgTypeTimer: {
      // Timer messages never change the state, they just cause an action (or
      // not)
      if ((currentState != fsmStateInit1Sent) &&
          (currentState != fsmStateInit2Sent)) {
        // Read in the values from the temperature sensor
        // We have three transactions on i2c to read the full temperature
        //   we send all three requests to the I2C thread (via a Queue) --
        // responses come back through the conductor thread
        // Temperature read -- use a convenient routine defined above
        if (vtI2CEnQ(devPtr,
                     vtI2CMsgTypeTempRead1,
                     0x4F,
                     sizeof(i2cCmdReadVals),
                     i2cCmdReadVals,
                     2) != pdTRUE) {
          VT_HANDLE_FATAL_ERROR(0);
        }
    ...
    case vtI2CMsgTypeTempRead3: {
      if (currentState == fsmStateTempRead3) {
        ...
        // Now have all of the values, so compute the temperature and send to
        // the LCD Task
        ...
        //#if PRINTF_VERSION == 1
        //#else
        // we do not have full printf (so no %f) and therefore need to print
        // out
        // integers
        // printf("Temp %d F (%d C)\n",lrint(32.0 + ((9.0/5.0)*temperature)),
        // lrint(temperature));
        // sprintf(lcdBuffer,"T=%d F (%d C)",lrint(32.0 +
        // ((9.0/5.0)*temperature)),lrint(temperature));
        // sprintf(lcdBuffer, "Count: %d", lrint(temperature));
        //#endif

        uint8_t lcdValue[1] = {value};
        if (lcdData != NULL) {
          if (SendLCDPrintMsg(lcdData,
                              strnlen(lcdBuffer, vtLCDMaxLen),
                              lcdBuffer,
