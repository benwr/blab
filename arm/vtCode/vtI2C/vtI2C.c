...
{
  uint8_t msgType;  // A field you will likely use in your communications
                    // between processors (and for debugging)
  uint8_t slvAddr;  // Address of the device to whom the message is being sent
                    // (or was sent)
  uint8_t rxLen;    // Length of the message you *expect* to receive (or, on the
                    // way back, the length that *was* received)
  uint8_t txLen;    // Length of the message you want to sent (or, on the way
                    // back, the length that *was* sent)
  uint8_t status;   // status of the completed operation -- I've not done
                    // anything much here, you probably should...
  uint8_t buf[vtI2CMLen];  // On the way in, message to be sent, on the way out,
                           // message received (if any)
} vtI2CMsg;
...
// Here is where we define an array of pointers that lets communication occur
// between the interrupt handler and the rest of the code in this file
static vtI2CStruct *devStaticPtr[3];

// I have set this to a large stack size because of (a) using printf() and (b)
// the depth of function calls for some of the I2C operations -- it is
// possible/very likely these are much larger than needed (see LCDtask.c for how
...
int vtI2CInit(vtI2CStruct *devPtr,
              uint8_t i2cDevNum,
              ...
              uint32_t i2cSpeed)
{
  ...
  case 0: {
    devStaticPtr[0] = devPtr;  // Setup the permanent variable for use by the
                               // interrupt handler
    devPtr->devAddr = LPC_I2C0;
    // Start with the interrupts disabled *and* make sure we have the priority
    // correct
  ...
  case 1: {
    devStaticPtr[1] = devPtr;  // Setup the permanent variable for use by the
                               // interrupt handler
  ...
  // Need to do an initial "take" on the semaphore to ensure that it is
  // initially blocked
  ...
  if ((retval = xTaskCreate(vI2CMonitorTask,
                            (signed char *)taskLabel,
                            i2cSTACK_SIZE,
                            (void *)devPtr,
                            devPtr->taskPriority,
                            (xTaskHandle *)NULL)) != pdPASS) {
    VT_HANDLE_FATAL_ERROR(retval);
    return (vtI2CErrInit);  // return is just to keep the compiler happy, we
                            // will never get here
  } else {
...
// A simple routine to use for filling out and sending a message to the I2C
// thread. You may want to make your own versions of these as they are not
// suited to all purposes
portBASE_TYPE vtI2CEnQ(vtI2CStruct *dev,
                       uint8_t msgType,
                       uint8_t slvAddr,
                       uint8_t txLen,
                       const uint8_t *txBuf,
...
portBASE_TYPE vtI2CDeQ(vtI2CStruct *dev,
                       uint8_t maxRxLen,
                       uint8_t *rxBuf,
                       uint8_t *rxLen,
                       uint8_t *msgType,
...
static __INLINE void
    vtI2CIsr(LPC_I2C_TypeDef *devAddr, xSemaphoreHandle *binSemaphore)
{
...
// Simply pass on the information to the real interrupt handler above (have to
// do this to work for multiple i2c peripheral units on the LPC1768
void vtI2C0Isr(void)
{
  // Log the I2C status code
  vtITMu8(vtITMPortI2C0IntHandler,
          ((devStaticPtr[0]->devAddr)->I2STAT & I2C_STAT_CODE_BITMASK));
...
{
  // Log the I2C status code
  vtITMu8(vtITMPortI2C1IntHandler,
          ((devStaticPtr[1]->devAddr)->I2STAT & I2C_STAT_CODE_BITMASK));
...
static portTASK_FUNCTION(vI2CMonitorTask, pvParameters)
{
  ...
  for (;;) {
    ...
    if (xQueueReceive(devPtr->inQ, (void *)&msgBuffer, portMAX_DELAY) !=
        pdTRUE) {
      VT_HANDLE_FATAL_ERROR(0);
    }
    ...
    transferMCfg.retransmissions_count = 0;  // this *should* be initialized in
                                             // the LPC code, but is not for
                                             // interrupt mode
    msgBuffer.status = I2C_MasterTransferData(
        devPtr->devAddr, &transferMCfg, I2C_TRANSFER_INTERRUPT);
    // Block until the I2C operation is complete -- we *cannot* overlap
    ...
    if (xQueueSend(devPtr->outQ, (void *)(&msgBuffer), portMAX_DELAY) !=
        pdTRUE) {
      // something went wrong
      VT_HANDLE_FATAL_ERROR(0);
    }
