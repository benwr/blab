...
%exp
%value = fread(obj, bytesAvailable, 'byte');

valuesReceived = obj.ValuesReceived;
%data.length = length(data) - 1;
%fprintf('callbackARMSimWiFly: Received data %d, %d bytes. Total of %d bytes read.\n',...
%    data, bytesAvailable, valuesReceived);

fprintf('Received data from sensor %d.\n Received data from sensor %d.\n Received data from sensor %d.\n', data, bytesAvailable, valuesReceived);
